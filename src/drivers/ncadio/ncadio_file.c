          ...
          int  *xtendimp)
{
    ...
    if (ndimsp != NULL){
        *ndimsp = 0;
        for (i = 0; i < ncadp->fp->nvars; i++) {
            *ndimsp += ncadp->ndims[i];
        }
    }

    if (nvarsp != NULL){
        *nvarsp = ncadp->fp->nvars;
    }
